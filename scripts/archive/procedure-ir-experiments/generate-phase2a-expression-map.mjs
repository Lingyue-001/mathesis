import fs from "node:fs/promises";
import { readJson, writeJson } from "./cullen-oracle-common.mjs";

const INPUT_PILOT = "tmp/procedure-ir/phase2a-procedure-ir-pilot.json";
const INPUT_RECON = "tmp/procedure-ir/cullen-led-source-reconstruction.json";
const INPUT_BASELINE = "tmp/procedure-ir/phase1-cullen-led-calibration-baseline.json";

const OUTPUT_MAP_JSON = "tmp/procedure-ir/phase2a-cullen-backed-expression-map.json";
const OUTPUT_MAP_MD = "tmp/procedure-ir/phase2a-cullen-backed-expression-map.md";
const OUTPUT_AUDIT_JSON = "tmp/procedure-ir/phase2a-expression-quality-audit.json";
const OUTPUT_AUDIT_MD = "tmp/procedure-ir/phase2a-expression-quality-audit.md";

const TARGET_PROC_IDS = ["Proc. 2.3", "Proc. 2.4", "Proc. 2.5", "Proc. 2.9", "Proc. 3.2"];

const ENTRY_SPECS = [
  {
    proc_id: "Proc. 2.3",
    step_id: "Proc.2.3.step.1",
    source_phrase: "以月法乘积月",
    cullen_chinese_phrase: "以月法乘積月",
    cullen_translation_phrase: "Multiply Accumulated Months by Lunation Factor [2392].",
    cullen_commentary_phrase: "Here the aim is to predict the cyclical day on which falls the conjunction of the first day of the first month of the Celestial year.",
    cullen_worked_example: "24 × 2392 = 57,408.",
    backup_level: "A_worked_example_formula",
    operation_id: "multiply_accumulated_months_by_lunation_factor",
    formal_expression: "product = 积月 × 月法",
    variables: ["积月"],
    constants: [{ name: "月法", value: 2392 }],
    input_bindings: { 积月: "pilot.inputs[source_text]: 积月", 月法: "Cullen translation constant [2392]" },
    output_bindings: { product: "intermediate quantity before Day Factor division" },
    validation_rule: "Check Cullen worked example: when 积月 = 24, product must equal 57,408.",
    formalization_status: "formalizable_now",
    uncertainties: [],
  },
  {
    proc_id: "Proc. 2.3",
    step_id: "Proc.2.3.step.2",
    source_phrase: "盈日法得一，名曰积日，不盈者名曰小馀",
    cullen_chinese_phrase: "盈日法得一，名曰積日，不盈者名曰小餘",
    cullen_translation_phrase: "Count 1 for each filling of the Day Factor [81], and the name of this is Accumulated Days. What does not fill is called the Lesser Remainder.",
    cullen_commentary_phrase: "57,408/81 = 708 remainder 60.",
    cullen_worked_example: "57,408/81 = 708 remainder 60.",
    backup_level: "A_worked_example_formula",
    operation_id: "divide_product_by_day_factor_to_get_accumulated_days_and_lesser_remainder",
    formal_expression: "积日 = floor(product / 日法); 小馀 = product mod 日法",
    variables: ["product"],
    constants: [{ name: "日法", value: 81 }],
    input_bindings: { product: "output of Proc.2.3.step.1", 日法: "Cullen translation constant [81]" },
    output_bindings: { 积日: "source output name", 小馀: "source output name" },
    validation_rule: "Check Cullen worked example: floor(57,408 / 81) = 708 and 57,408 mod 81 = 60.",
    formalization_status: "formalizable_now",
    uncertainties: [],
  },
  {
    proc_id: "Proc. 2.3",
    step_id: "Proc.2.3.step.3",
    source_phrase: "小馀三十八以上，其月大",
    cullen_chinese_phrase: "小餘三十八以上，其月大",
    cullen_translation_phrase: "If the Lesser Remainder is 38 or above, the month is long.",
    cullen_commentary_phrase: "Since 38 + 43 = 81 it is clear that if the Lesser Remainder at the start of this month is 43 or more, then a whole extra day will have to be counted before we get to the next conjunction.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "classify_long_month_from_lesser_remainder_threshold",
    formal_expression: "is_long_month = (小馀 >= 38)",
    variables: ["小馀"],
    constants: [{ name: "long_month_threshold", value: 38 }],
    input_bindings: { 小馀: "output of Proc.2.3.step.2" },
    output_bindings: { is_long_month: "month-length classification flag" },
    validation_rule: "Evaluate the predicate only after 小馀 is resolved from the Day Factor quotient/remainder step.",
    formalization_status: "formalizable_with_caveat",
    uncertainties: ["Cullen commentary explains the carry implication for later month advance, but not a separate standalone month-length algorithm beyond this threshold test."],
  },
  {
    proc_id: "Proc. 2.3",
    step_id: "Proc.2.3.step.4",
    source_phrase: "积日盈六十，除之，不盈者名曰大馀",
    cullen_chinese_phrase: "積日盈六十，除之，不盈者名曰大餘",
    cullen_translation_phrase: "If the Accumulated Days fills 60, cast it out. What does not fill is called the Greater Remainder.",
    cullen_commentary_phrase: "708/60 = 11 remainder 48. So 11 complete 60-day cycles and 48 odd days have elapsed since the start of the current concordance, and thus 48 is the Greater Remainder.",
    cullen_worked_example: "708/60 = 11 remainder 48.",
    backup_level: "A_worked_example_formula",
    operation_id: "cast_out_sixty_from_accumulated_days_to_get_greater_remainder",
    formal_expression: "大馀 = 积日 mod 60",
    variables: ["积日"],
    constants: [{ name: "sexagenary_cycle", value: 60 }],
    input_bindings: { 积日: "output of Proc.2.3.step.2" },
    output_bindings: { 大馀: "source output name" },
    validation_rule: "Check Cullen worked example: 708 mod 60 = 48.",
    formalization_status: "formalizable_now",
    uncertainties: [],
  },
  {
    proc_id: "Proc. 2.3",
    step_id: "Proc.2.3.step.5",
    source_phrase: "数从统首日起，算外，则朔日也",
    cullen_chinese_phrase: "數從統首日起，算外，則朔日也",
    cullen_translation_phrase: "Count starting from the Concordance Head, and outside the count is the day of conjunction.",
    cullen_commentary_phrase: "If the first day is number 1 and we are to count 48, the day 'outside the count', the day of conjunction, is number 49.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "count_outside_from_concordance_head_to_conjunction_day",
    formal_expression: "朔日_index = ((统首日起点_index - 1) + 大馀) mod 60 + 1",
    variables: ["统首日起点_index", "大馀"],
    constants: [{ name: "sexagenary_cycle", value: 60 }],
    input_bindings: { 统首日起点_index: "requires external cyclical-day index for 统首日", 大馀: "output of Proc.2.3.step.4" },
    output_bindings: { 朔日_index: "cyclical-day position of conjunction day" },
    validation_rule: "Only validate when a concrete cyclical-day index for 统首日 is supplied; use Cullen's 48 -> 49 counting explanation as the convention check.",
    formalization_status: "formalizable_with_caveat",
    uncertainties: ["This remains a counting convention tied to a cyclical-day index, not a pure arithmetic step recoverable from the source phrase alone."],
  },
  {
    proc_id: "Proc. 2.3",
    step_id: "Proc.2.3.step.6",
    source_phrase: "求其次月，加大馀二十九，小馀四十三",
    cullen_chinese_phrase: "求其次月，加大餘二十九，小餘四十三",
    cullen_translation_phrase: "To seek the next month, add to the Greater Remainder 29, and to the Lesser Remainder 43.",
    cullen_commentary_phrase: "Since the mean interval between conjunctions will be 2392/81 days = 29 + 43/81 days it is clear that 29 and 43 are the amounts we must add to the Greater and Lesser Remainders to get to the next conjunction.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "advance_remainder_pair_to_next_month",
    formal_expression: "next_month_greater_raw = 大馀 + 29; next_month_lesser_raw = 小馀 + 43",
    variables: ["大馀", "小馀"],
    constants: [{ name: "next_month_increment_days", value: 29 }, { name: "next_month_increment_parts", value: 43 }],
    input_bindings: { 大馀: "output of Proc.2.3.step.4", 小馀: "output of Proc.2.3.step.2" },
    output_bindings: { next_month_greater_raw: "pre-carry greater remainder", next_month_lesser_raw: "pre-carry lesser remainder" },
    validation_rule: "Validate the 29 + 43/81 decomposition against Cullen's commentary relation 2392/81 = 29 + 43/81.",
    formalization_status: "formalizable_now",
    uncertainties: [],
  },
  {
    proc_id: "Proc. 2.3",
    step_id: "Proc.2.3.step.7",
    source_phrase: "小馀盈日法得一，从大馀，数除如法",
    cullen_chinese_phrase: "小餘盈日法得一，從大餘，數除如法",
    cullen_translation_phrase: "Count one for each time the Lesser Remainder fills the Day Factor [81], and let it go with the Greater Remainder. Then count and cast out according to the method.",
    cullen_commentary_phrase: "If the Lesser Remainder at the start of this month is 43 or more, then a whole extra day will have to be counted before we get to the next conjunction.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "carry_lesser_remainder_into_greater_remainder_for_next_month",
    formal_expression: "next_month_greater = next_month_greater_raw + floor(next_month_lesser_raw / 日法); next_month_lesser = next_month_lesser_raw mod 日法",
    variables: ["next_month_greater_raw", "next_month_lesser_raw"],
    constants: [{ name: "日法", value: 81 }],
    input_bindings: { next_month_greater_raw: "output of Proc.2.3.step.6", next_month_lesser_raw: "output of Proc.2.3.step.6", 日法: "Cullen translation constant [81]" },
    output_bindings: { next_month_greater: "carried greater remainder", next_month_lesser: "carried lesser remainder" },
    validation_rule: "Validate carry behavior only through the Day Factor division rule; do not collapse the later '数除如法' convention into this arithmetic substep.",
    formalization_status: "formalizable_with_caveat",
    uncertainties: ["The final phrase 数除如法 still points back to an external counting convention, so only the carry arithmetic is formalized here."],
  },
  {
    proc_id: "Proc. 2.3",
    step_id: "Proc.2.3.step.8",
    source_phrase: "求弦，加大馀七，小馀三十一",
    cullen_chinese_phrase: "求弦，加大餘七，小餘三十一",
    cullen_translation_phrase: "To seek the first quarter, add to the Greater Remainder 7, and to the Lesser Remainder 31.",
    cullen_commentary_phrase: "2392/4 = 598 and 598/81 = 7 remainder 31.",
    cullen_worked_example: "2392/4 = 598 and 598/81 = 7 remainder 31.",
    backup_level: "A_worked_example_formula",
    operation_id: "advance_remainder_pair_to_first_quarter",
    formal_expression: "first_quarter_greater_raw = 大馀 + 7; first_quarter_lesser_raw = 小馀 + 31",
    variables: ["大馀", "小馀"],
    constants: [{ name: "first_quarter_increment_days", value: 7 }, { name: "first_quarter_increment_parts", value: 31 }],
    input_bindings: { 大馀: "output of Proc.2.3.step.4", 小馀: "output of Proc.2.3.step.2" },
    output_bindings: { first_quarter_greater_raw: "pre-carry first-quarter greater remainder", first_quarter_lesser_raw: "pre-carry first-quarter lesser remainder" },
    validation_rule: "Check Cullen worked example: 2392/4 = 598 and 598 = 7 × 81 + 31.",
    formalization_status: "formalizable_now",
    uncertainties: [],
  },
  {
    proc_id: "Proc. 2.3",
    step_id: "Proc.2.3.step.9",
    source_phrase: "求望，倍弦",
    cullen_chinese_phrase: "求望，倍弦",
    cullen_translation_phrase: "To seek full moon, double [the amounts] for the first quarter.",
    cullen_commentary_phrase: "It is assumed that the lunar phases are equally spaced.",
    cullen_worked_example: "",
    backup_level: "C_translation_explicit_operation",
    operation_id: "double_first_quarter_increment_to_get_full_moon_increment",
    formal_expression: "full_moon_increment = 2 × first_quarter_increment",
    variables: ["first_quarter_increment"],
    constants: [],
    input_bindings: { first_quarter_increment: "paired increment defined in Proc.2.3.step.8" },
    output_bindings: { full_moon_increment: "paired increment for full moon" },
    validation_rule: "Validate only after the first-quarter increment pair is resolved; the doubling rule itself is explicit in Cullen's translation.",
    formalization_status: "formalizable_now",
    uncertainties: ["The source does not restate whether both greater and lesser components are doubled, but Cullen's translation supports doubling the first-quarter amounts as a pair."],
  },
  {
    proc_id: "Proc. 2.4",
    step_id: "Proc.2.4.step.1",
    source_phrase: "以十二乘闰馀",
    cullen_chinese_phrase: "以十二乘閏餘",
    cullen_translation_phrase: "Multiply the Intercalation Surplus by 12.",
    cullen_commentary_phrase: "The initial multiplication by 12 is needed to convert the Intercalation Surplus to the correct scale, since 12 × 19 = 228.",
    cullen_worked_example: "14 × 12 = 168.",
    backup_level: "A_worked_example_formula",
    operation_id: "rescale_intercalation_surplus_by_twelve",
    formal_expression: "rescaled_surplus = 闰馀 × 12",
    variables: ["闰馀"],
    constants: [{ name: "multiplier", value: 12 }],
    input_bindings: { 闰馀: "pilot.inputs: 闰馀" },
    output_bindings: { rescaled_surplus: "Intercalation Surplus on the 228-part scale" },
    validation_rule: "Check Cullen worked example: when 闰馀 = 14, rescaled_surplus = 168.",
    formalization_status: "formalizable_now",
    uncertainties: [],
  },
  {
    proc_id: "Proc. 2.4",
    step_id: "Proc.2.4.step.2a",
    source_phrase: "加十得一",
    cullen_chinese_phrase: "加(十)[七]得一",
    cullen_translation_phrase: "For an addition of seven, obtain 1, [until you] fill Rule Medial [Qi] [228].",
    cullen_commentary_phrase: "We then find how many times we need to add 7 to this until we reach a total exceeding 228.",
    cullen_worked_example: "14 × 12 = 168 and (228 − 168) / 7 = 8, remainder 4.",
    backup_level: "D_source_only_needs_human_review",
    operation_id: "source_increment_ten_discrepancy_before_medial_qi_fill",
    formal_expression: null,
    variables: ["闰馀"],
    constants: [{ name: "source_increment", value: 10 }, { name: "cullen_emended_increment", value: 7 }],
    input_bindings: { 闰馀: "pilot.inputs: 闰馀" },
    output_bindings: {},
    validation_rule: "Do not validate as a deterministic formula until the source/Cullen discrepancy is resolved by human review.",
    formalization_status: "not_formalizable_yet",
    uncertainties: ["The source text says 加十, while Cullen's reconstructed Chinese and translation support 加七; this entry preserves the discrepancy without normalization."],
  },
  {
    proc_id: "Proc. 2.4",
    step_id: "Proc.2.4.step.2b",
    source_phrase: "加十得一",
    cullen_chinese_phrase: "加(十)[七]得一",
    cullen_translation_phrase: "For an addition of seven, obtain 1, [until you] fill Rule Medial [Qi] [228].",
    cullen_commentary_phrase: "If we start counting with winter solstice taken as #1, and if (say) 3 additions of 7 are required, the 4th qi will be the one 'outside the count' that has shifted onto or past the next mean conjunction.",
    cullen_worked_example: "(228 − 168) / 7 = 8, remainder 4.",
    backup_level: "A_worked_example_formula",
    operation_id: "compute_number_of_sevens_needed_to_cross_rule_medial_qi_cycle",
    formal_expression: "steps_before_overflow = floor((章中 − rescaled_surplus) / 7)",
    variables: ["rescaled_surplus"],
    constants: [{ name: "章中", value: 228 }, { name: "cullen_increment", value: 7 }],
    input_bindings: { rescaled_surplus: "output of Proc.2.4.step.1", 章中: "Cullen translation constant [228]" },
    output_bindings: { steps_before_overflow: "number of completed 7-unit advances before overflow" },
    validation_rule: "Check Cullen worked example: floor((228 − 168) / 7) = 8 with remainder 4.",
    formalization_status: "formalizable_with_caveat",
    uncertainties: ["This formalization is Cullen-backed but cannot overwrite the source's 加十 reading."],
  },
  {
    proc_id: "Proc. 2.4",
    step_id: "Proc.2.4.step.3",
    source_phrase: "盈章中，数所得，起冬至，算外，则中至终闰盈",
    cullen_chinese_phrase: "盈章中，數所得，起冬至，算外，則中至終閏盈",
    cullen_translation_phrase: "With the number you get, start counting off from winter solstice. Outside the count, then the Medial [Qi] has reached the conclusion of the filling of the Intercalation [Surplus].",
    cullen_commentary_phrase: "If 3 additions of 7 are required, the 4th qi will be the one 'outside the count' that has shifted onto or past the next mean conjunction.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "map_overflow_step_count_to_medial_qi_outside_the_count",
    formal_expression: "overflow_qi_index = steps_before_overflow + 1",
    variables: ["steps_before_overflow"],
    constants: [],
    input_bindings: { steps_before_overflow: "output of Proc.2.4.step.2b" },
    output_bindings: { overflow_qi_index: "ordinal medial-qi index counted from winter solstice as #1" },
    validation_rule: "Validate only as an ordinal counting convention: if 3 additions are required, the 4th qi is outside the count.",
    formalization_status: "formalizable_with_caveat",
    uncertainties: ["This step formalizes only Cullen's ordinal explanation, not a full calendrical date computation."],
  },
  {
    proc_id: "Proc. 2.4",
    step_id: "Proc.2.4.step.4",
    source_phrase: "中气在朔若二日，则前月闰也",
    cullen_chinese_phrase: "中氣在朔若二日，則前月閏也",
    cullen_translation_phrase: "The Medial [Qi] is on the conjunction or the second day, so the preceding month is intercalary.",
    cullen_commentary_phrase: "This qi could fall on the day after the day of the conjunction, if the conjunction falls more than 0.22 day after midnight on the first day of the month.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "classify_preceding_month_as_intercalary_from_medial_qi_timing",
    formal_expression: "preceding_month_is_intercalary = (medial_qi_day == conjunction_day) or (medial_qi_day == conjunction_day + 1)",
    variables: ["medial_qi_day", "conjunction_day"],
    constants: [],
    input_bindings: { medial_qi_day: "requires calendrical day placement of the overflow qi", conjunction_day: "requires conjunction-day placement in the target month" },
    output_bindings: { preceding_month_is_intercalary: "intercalary classification flag" },
    validation_rule: "Validate only when conjunction-day and medial-qi day placements are both available; do not infer them from this line alone.",
    formalization_status: "formalizable_with_caveat",
    uncertainties: ["This remains dependent on calendrical placement data outside the source phrase itself."],
  },
  {
    proc_id: "Proc. 2.5",
    step_id: "Proc.2.5.step.1",
    source_phrase: "以算馀乘入统岁数",
    cullen_chinese_phrase: "以(算)[策]餘乘(人)[入]統歲數",
    cullen_translation_phrase: "By Reckoning Surplus [8080] multiply the number of years into the Concordance.",
    cullen_commentary_phrase: "The number 8080 is the number of days by which the days in a Concordance of 1539 years exceed a multiple of 360.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "multiply_years_into_concordance_by_reckoning_surplus",
    formal_expression: "winter_solstice_product = 入统岁数 × 算馀",
    variables: ["入统岁数"],
    constants: [{ name: "算馀", value: 8080 }],
    input_bindings: { 入统岁数: "pilot.inputs: 入统岁数", 算馀: "Cullen translation constant [8080]" },
    output_bindings: { winter_solstice_product: "intermediate quantity before Concordance Factor division" },
    validation_rule: "Validate only after the Reckoning Surplus constant is accepted from Cullen's commentary.",
    formalization_status: "formalizable_now",
    uncertainties: ["Cullen gives the constant's meaning but not a worked numerical example for this specific procedure."],
  },
  {
    proc_id: "Proc. 2.5",
    step_id: "Proc.2.5.step.2",
    source_phrase: "盈统法得一，名曰大馀，不盈者名曰小馀",
    cullen_chinese_phrase: "盈統法得一，名曰大餘，不盈者名曰小餘",
    cullen_translation_phrase: "Obtaining 1 for each filling of the Concordance Factor [1539]. [This] is called the Greater Remainder. What does not fill is called the Lesser Remainder.",
    cullen_commentary_phrase: "The rest follows.",
    cullen_worked_example: "",
    backup_level: "C_translation_explicit_operation",
    operation_id: "divide_winter_solstice_product_by_concordance_factor",
    formal_expression: "大馀 = floor(winter_solstice_product / 统法); 小馀 = winter_solstice_product mod 统法",
    variables: ["winter_solstice_product"],
    constants: [{ name: "统法", value: 1539 }],
    input_bindings: { winter_solstice_product: "output of Proc.2.5.step.1", 统法: "Cullen translation constant [1539]" },
    output_bindings: { 大馀: "source output name", 小馀: "source output name" },
    validation_rule: "Validate quotient/remainder only after the multiplier step is accepted; no Cullen worked example is available in the allowed inputs.",
    formalization_status: "formalizable_with_caveat",
    uncertainties: ["The translation is explicit about quotient/remainder behavior, but the allowed inputs do not include a worked numerical example for this step."],
  },
  {
    proc_id: "Proc. 2.5",
    step_id: "Proc.2.5.step.3",
    source_phrase: "除数如法，则所求冬至日也",
    cullen_chinese_phrase: "除數如法，則所求冬至日也",
    cullen_translation_phrase: "Cast out and count off according to the method, then that is the winter solstice day of the year sought.",
    cullen_commentary_phrase: "The rest follows.",
    cullen_worked_example: "",
    backup_level: "C_translation_explicit_operation",
    operation_id: "recover_winter_solstice_day_by_prior_counting_method",
    formal_expression: null,
    variables: ["大馀", "小馀"],
    constants: [],
    input_bindings: { 大馀: "output of Proc.2.5.step.2", 小馀: "output of Proc.2.5.step.2" },
    output_bindings: { 冬至日: "source output name" },
    validation_rule: "Do not run arithmetic validation until the prior counting convention referenced by 如法 is explicitly grounded.",
    formalization_status: "needs_human_review",
    uncertainties: ["除数如法 points to an external convention not formalized by Cullen in the allowed input scope."],
  },
  {
    proc_id: "Proc. 2.9",
    step_id: "Proc.2.9.step.1",
    source_phrase: "以章岁乘月小馀",
    cullen_chinese_phrase: "以章歲乘月小餘",
    cullen_translation_phrase: "Multiply the Lesser Remainder of the month by Rule Years [19].",
    cullen_commentary_phrase: "Multiplying this by 19 converts to a scale of 19 × 81 = 1539, the Concordance Factor, used for fractions of a du.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "convert_month_lesser_remainder_to_du_fraction_scale",
    formal_expression: "midnight_offset_parts = 月小馀 × 章岁",
    variables: ["月小馀"],
    constants: [{ name: "章岁", value: 19 }],
    input_bindings: { 月小馀: "pilot.inputs: 月小馀", 章岁: "Cullen translation constant [19]" },
    output_bindings: { midnight_offset_parts: "offset on the 1539-part du scale" },
    validation_rule: "Check the scale-conversion relation 19 × 81 = 1539 before using this offset in subtraction.",
    formalization_status: "formalizable_now",
    uncertainties: [],
  },
  {
    proc_id: "Proc. 2.9",
    step_id: "Proc.2.9.step.2",
    source_phrase: "以减合晨度",
    cullen_chinese_phrase: "以減合晨度",
    cullen_translation_phrase: "And by that diminish the du of the mark-point for the conjunction.",
    cullen_commentary_phrase: "Subtracting this amount from the du for the conjunction gives the du for the preceding midnight.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "subtract_midnight_offset_from_conjunction_mark_point",
    formal_expression: "日夜半所在度 = 合晨度 − (midnight_offset_parts / 统法)",
    variables: ["合晨度", "midnight_offset_parts"],
    constants: [{ name: "统法", value: 1539 }],
    input_bindings: { 合晨度: "pilot.inputs: 合晨度", midnight_offset_parts: "output of Proc.2.9.step.1", 统法: "Cullen commentary relation 19 × 81 = 1539" },
    output_bindings: { 日夜半所在度: "sun midnight du position" },
    validation_rule: "Validate only on a common du/part scale; do not collapse lodge indexing into this expression.",
    formalization_status: "formalizable_with_caveat",
    uncertainties: ["This formalizes the du/part subtraction, not the full lodge-plus-du representation."],
  },
  {
    proc_id: "Proc. 2.9",
    step_id: "Proc.2.9.step.3",
    source_phrase: "小馀不足者，破全度",
    cullen_chinese_phrase: "小餘不足者，破全度",
    cullen_translation_phrase: "If the Lesser Remainder is insufficient, break a whole du [into parts].",
    cullen_commentary_phrase: "The Lesser Remainder for a month is a fraction of a day at a scale of 81... used for fractions of a du.",
    cullen_worked_example: "",
    backup_level: "C_translation_explicit_operation",
    operation_id: "borrow_one_whole_du_into_parts_before_subtraction",
    formal_expression: "if midnight_offset_parts > conjunction_fractional_parts then borrow_du = 1 and conjunction_fractional_parts = conjunction_fractional_parts + 统法",
    variables: ["midnight_offset_parts", "conjunction_fractional_parts"],
    constants: [{ name: "统法", value: 1539 }],
    input_bindings: { midnight_offset_parts: "output of Proc.2.9.step.1", conjunction_fractional_parts: "fractional part of 合晨度", 统法: "Cullen commentary relation 19 × 81 = 1539" },
    output_bindings: { borrow_du: "borrow flag", conjunction_fractional_parts: "adjusted fractional parts after borrowing" },
    validation_rule: "Validate only after the conjunction mark-point is represented on the same 1539-part scale.",
    formalization_status: "formalizable_with_caveat",
    uncertainties: ["The source does not spell out the exact bookkeeping of the borrowed whole du, so the expression captures only the necessary borrow condition."],
  },
  {
    proc_id: "Proc. 3.2",
    step_id: "Proc.3.2.step.1",
    source_phrase: "以元法除去上元",
    cullen_chinese_phrase: "以元法除去上元",
    cullen_translation_phrase: "Cast out Origin Factor [4560] from accumulated years from Grand Origin.",
    cullen_commentary_phrase: "First we cast out whole Origins, since all relevant conditions repeat in an Origin.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "cast_out_whole_origins_from_accumulated_years",
    formal_expression: "origin_remainder = 上元积年 mod 元法",
    variables: ["上元积年"],
    constants: [{ name: "元法", value: 4560 }],
    input_bindings: { 上元积年: "pilot.inputs: 上元积年", 元法: "Cullen translation constant [4560]" },
    output_bindings: { origin_remainder: "remainder after removing whole Origins" },
    validation_rule: "Validate only as the first nested cycle cast-out in the benchmark procedure.",
    formalization_status: "formalizable_now",
    uncertainties: [],
  },
  {
    proc_id: "Proc. 3.2",
    step_id: "Proc.3.2.step.2",
    source_phrase: "其馀以纪法除之",
    cullen_chinese_phrase: "其餘以紀法除之",
    cullen_translation_phrase: "Cast out Era Factor [1520] from the remainder.",
    cullen_commentary_phrase: "Since an Origin is three Eras, we then establish which of the three Eras of the current Origin we are in.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "cast_out_era_factor_from_origin_remainder",
    formal_expression: "entered_era_index = floor(origin_remainder / 纪法); 入纪年数 = origin_remainder mod 纪法",
    variables: ["origin_remainder"],
    constants: [{ name: "纪法", value: 1520 }],
    input_bindings: { origin_remainder: "output of Proc.3.2.step.1", 纪法: "Cullen translation constant [1520]" },
    output_bindings: { entered_era_index: "ordinal Era index within current Origin", 入纪年数: "years entered into the Era" },
    validation_rule: "Validate only as a nested quotient/remainder relation; do not yet convert the quotient into table lookup behavior.",
    formalization_status: "formalizable_now",
    uncertainties: ["The quotient's conversion to a named Era sequence is intentionally left to a later benchmark-only convention layer."],
  },
  {
    proc_id: "Proc. 3.2",
    step_id: "Proc.3.2.step.3",
    source_phrase: "以蔀法除之",
    cullen_chinese_phrase: "以蔀法除之",
    cullen_translation_phrase: "Cast out Obscuration Factor [76] from it.",
    cullen_commentary_phrase: "Each column has 20 rows, containing the sexagenary numbers of the first year of each of the 20 Obscurations that make up the relevant Era.",
    cullen_worked_example: "",
    backup_level: "C_translation_explicit_operation",
    operation_id: "cast_out_obscuration_factor_from_years_into_era",
    formal_expression: "entered_obscuration_index = floor(入纪年数 / 蔀法); 入蔀年数 = 入纪年数 mod 蔀法",
    variables: ["入纪年数"],
    constants: [{ name: "蔀法", value: 76 }],
    input_bindings: { 入纪年数: "output of Proc.3.2.step.2", 蔀法: "Cullen translation constant [76]" },
    output_bindings: { entered_obscuration_index: "ordinal Obscuration index within the Era", 入蔀年数: "years entered into the Obscuration" },
    validation_rule: "Validate only as the third nested quotient/remainder relation in the benchmark procedure.",
    formalization_status: "formalizable_now",
    uncertainties: [],
  },
  {
    proc_id: "Proc. 3.2",
    step_id: "Proc.3.2.step.4",
    source_phrase: "所得数从天纪，算外则所入纪也",
    cullen_chinese_phrase: "所得數從天紀，筭外則所入紀也",
    cullen_translation_phrase: "Number from the Heaven Era by how many you obtain, then outside the count is the Era you are entering.",
    cullen_commentary_phrase: "That tells us which of the three main columns ... we should be using.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "map_era_quotient_to_heaven_era_counting_convention",
    formal_expression: null,
    variables: ["entered_era_index"],
    constants: [],
    input_bindings: { entered_era_index: "output of Proc.3.2.step.2" },
    output_bindings: { 所入纪: "named Era selected by Heaven Era counting convention" },
    validation_rule: "Benchmark only: do not arithmetic-validate until the Heaven/Earth/Man table convention is encoded separately.",
    formalization_status: "needs_human_review",
    uncertainties: ["This is a named-sequence/table convention, not a standalone arithmetic relation under the current pilot scope."],
  },
  {
    proc_id: "Proc. 3.2",
    step_id: "Proc.3.2.step.5",
    source_phrase: "所得数从甲子蔀起，算外，所入蔀也",
    cullen_chinese_phrase: "所得數從甲子蔀起，筭外，[所入蔀也]",
    cullen_translation_phrase: "Number from the jiazi.1 Obscuration by how many you obtain, then outside your count, that is the Obscuration which is being entered.",
    cullen_commentary_phrase: "Each column has 20 rows, containing the sexagenary numbers of the first year of each of the 20 Obscurations that make up the relevant Era.",
    cullen_worked_example: "",
    backup_level: "C_translation_explicit_operation",
    operation_id: "map_obscuration_quotient_to_jiazi_obscuration_counting_convention",
    formal_expression: null,
    variables: ["entered_obscuration_index"],
    constants: [],
    input_bindings: { entered_obscuration_index: "output of Proc.3.2.step.3" },
    output_bindings: { 所入蔀: "named Obscuration selected by jiazi counting convention" },
    validation_rule: "Benchmark only: do not arithmetic-validate until the row/column lookup convention is encoded separately.",
    formalization_status: "needs_human_review",
    uncertainties: ["This remains a table-mediated naming step rather than a direct arithmetic expression."],
  },
  {
    proc_id: "Proc. 3.2",
    step_id: "Proc.3.2.step.6",
    source_phrase: "岁名命之，算上，即所求年太岁所在",
    cullen_chinese_phrase: "歲名命之，筭上，即所求年太歲所在",
    cullen_translation_phrase: "Label that by the year-name ... then above the count, that is where the Great Year is in the year sought.",
    cullen_commentary_phrase: "We do this by finding the sexagenary number of the first year of the current Obscuration, and then counting forward.",
    cullen_worked_example: "",
    backup_level: "B_commentary_explicit_relation",
    operation_id: "count_forward_from_obscuration_entry_to_target_year_name",
    formal_expression: null,
    variables: ["入蔀年数"],
    constants: [],
    input_bindings: { 入蔀年数: "output of Proc.3.2.step.3" },
    output_bindings: { 所求年太岁所在: "target-year sexagenary year-name position" },
    validation_rule: "Keep as benchmark-only human review until the Table 3.1 lookup and counting-above convention are formalized separately.",
    formalization_status: "needs_human_review",
    uncertainties: ["This step depends on a table-driven sequence and should not be generalized from the Sifen benchmark to Santong examples."],
  },
];

function makeSnippet(text, length = 240) {
  const normalized = String(text ?? "").replace(/\s+/gu, " ").trim();
  return normalized.length <= length ? normalized : `${normalized.slice(0, length)}...`;
}

function unique(values) {
  return [...new Set(values)];
}

function ensure(condition, message) {
  if (!condition) throw new Error(message);
}

function groupBy(entries, key) {
  const map = new Map();
  for (const entry of entries) {
    const value = entry[key];
    const list = map.get(value) ?? [];
    list.push(entry);
    map.set(value, list);
  }
  return map;
}

function renderExpressionMapMarkdown(entries) {
  const lines = [
    "# Phase 2A-2 Cullen-Backed Expression Map",
    "",
    `- Scope: ${TARGET_PROC_IDS.join(", ")}`,
    "- Input restriction respected: phase2a pilot + Cullen-led reconstruction + phase1 baseline only",
    "- Original Phase 2A pilot remains unchanged",
    "",
  ];

  const byProc = groupBy(entries, "proc_id");
  for (const procId of TARGET_PROC_IDS) {
    const procEntries = byProc.get(procId) ?? [];
    lines.push(`## ${procId}`, "");
    for (const entry of procEntries) {
      lines.push(`### ${entry.step_id}`);
      lines.push("");
      lines.push(`- backup_level: ${entry.backup_level}`);
      lines.push(`- operation_id: ${entry.operation_id}`);
      lines.push(`- formalization_status: ${entry.formalization_status}`);
      lines.push(`- source_phrase: ${entry.source_phrase}`);
      lines.push(`- cullen_chinese_phrase: ${entry.cullen_chinese_phrase}`);
      lines.push(`- cullen_translation_phrase: ${entry.cullen_translation_phrase}`);
      lines.push(`- cullen_commentary_phrase: ${entry.cullen_commentary_phrase}`);
      lines.push(`- cullen_worked_example: ${entry.cullen_worked_example || "none"}`);
      lines.push(`- formal_expression: ${entry.formal_expression || "null"}`);
      lines.push(`- validation_rule: ${entry.validation_rule}`);
      lines.push("");
      if (entry.uncertainties.length) {
        lines.push("Uncertainties:");
        for (const item of entry.uncertainties) {
          lines.push(`- ${item}`);
        }
        lines.push("");
      }
    }
  }

  return `${lines.join("\n")}\n`;
}

function renderAuditMarkdown(audit) {
  const lines = [
    "# Phase 2A-2 Expression Quality Audit",
    "",
    "## Summary",
    "",
    `- total_entries: ${audit.total_entries}`,
    `- A/B backup entries: ${audit.backup_summary.A_worked_example_formula + audit.backup_summary.B_commentary_explicit_relation}`,
    `- C/D backup entries: ${audit.backup_summary.C_translation_explicit_operation + audit.backup_summary.D_source_only_needs_human_review}`,
    `- formalizable_now: ${audit.formalization_summary.formalizable_now}`,
    `- formalizable_with_caveat: ${audit.formalization_summary.formalizable_with_caveat}`,
    `- not_formalizable_yet: ${audit.formalization_summary.not_formalizable_yet}`,
    `- needs_human_review: ${audit.formalization_summary.needs_human_review}`,
    "",
    "## A/B Backup",
    "",
  ];

  for (const line of audit.expressions_with_AB_backup) lines.push(`- ${line}`);
  lines.push("", "## C/D Backup", "");
  for (const line of audit.expressions_with_CD_backup) lines.push(`- ${line}`);
  lines.push("", "## Operation IDs Needing Rename", "");
  for (const line of audit.operation_ids_need_rename) lines.push(`- ${line}`);
  lines.push("", "## Evidence-Insufficient Formal Expressions", "");
  for (const line of audit.formal_expressions_with_insufficient_evidence) lines.push(`- ${line}`);
  lines.push("", "## Arithmetic Validation Readiness", "");
  for (const line of audit.procs_ready_for_arithmetic_validation) lines.push(`- ${line}`);
  lines.push("", "## Human Review Only", "");
  for (const line of audit.procs_limited_to_human_review) lines.push(`- ${line}`);
  lines.push("", "## Schema Recommendation", "");
  lines.push(`- ${audit.current_pilot_schema_regenerate}`);
  lines.push("");
  return `${lines.join("\n")}\n`;
}

function buildAudit(entries) {
  const backupSummary = {
    A_worked_example_formula: entries.filter((item) => item.backup_level === "A_worked_example_formula").length,
    B_commentary_explicit_relation: entries.filter((item) => item.backup_level === "B_commentary_explicit_relation").length,
    C_translation_explicit_operation: entries.filter((item) => item.backup_level === "C_translation_explicit_operation").length,
    D_source_only_needs_human_review: entries.filter((item) => item.backup_level === "D_source_only_needs_human_review").length,
  };

  const formalizationSummary = {
    formalizable_now: entries.filter((item) => item.formalization_status === "formalizable_now").length,
    formalizable_with_caveat: entries.filter((item) => item.formalization_status === "formalizable_with_caveat").length,
    not_formalizable_yet: entries.filter((item) => item.formalization_status === "not_formalizable_yet").length,
    needs_human_review: entries.filter((item) => item.formalization_status === "needs_human_review").length,
  };

  const expressionsWithAB = entries
    .filter((item) => ["A_worked_example_formula", "B_commentary_explicit_relation"].includes(item.backup_level))
    .map((item) => `${item.step_id} | ${item.backup_level} | ${item.operation_id}`);

  const expressionsWithCD = entries
    .filter((item) => ["C_translation_explicit_operation", "D_source_only_needs_human_review"].includes(item.backup_level))
    .map((item) => `${item.step_id} | ${item.backup_level} | ${item.operation_id}`);

  const operationIdsNeedRename = entries
    .filter((item) => /count_outside|count_above|counting_convention|discrepancy|benchmark/i.test(item.operation_id))
    .map((item) => `${item.step_id} | ${item.operation_id} | convention-heavy or discrepancy-heavy naming should stay explicitly scoped`);

  const insufficientEvidence = entries
    .filter((item) => item.formal_expression === null || ["D_source_only_needs_human_review", "C_translation_explicit_operation"].includes(item.backup_level))
    .map((item) => `${item.step_id} | backup=${item.backup_level} | status=${item.formalization_status} | expression=${item.formal_expression ?? "null"}`);

  const byProc = groupBy(entries, "proc_id");
  const readyForArithmetic = [];
  const humanReviewOnly = [];

  for (const procId of TARGET_PROC_IDS) {
    const procEntries = byProc.get(procId) ?? [];
    const hasOnlyStrong = procEntries.every((item) => item.formal_expression && ["A_worked_example_formula", "B_commentary_explicit_relation", "C_translation_explicit_operation"].includes(item.backup_level));
    const hasNoHardBlock = procEntries.every((item) => !["D_source_only_needs_human_review", "needs_human_review", "not_formalizable_yet"].includes(item.backup_level) && !["needs_human_review", "not_formalizable_yet"].includes(item.formalization_status));

    if (procId === "Proc. 2.3") {
      readyForArithmetic.push(`${procId} | core quotient/remainder and increment substeps are machine-checkable; counting convention step still caveated.`);
      continue;
    }
    if (procId === "Proc. 2.5") {
      readyForArithmetic.push(`${procId} | multiplier and quotient/remainder substeps can enter arithmetic validation once 如法 counting remains excluded.`);
      continue;
    }
    if (procId === "Proc. 2.9") {
      readyForArithmetic.push(`${procId} | scale-conversion and subtraction pilot can be checked on a common parts scale, with borrow logic caveated.`);
      continue;
    }
    if (procId === "Proc. 2.4") {
      humanReviewOnly.push(`${procId} | source 加十 vs Cullen 加七 discrepancy blocks full arithmetic validation.`);
      continue;
    }
    if (procId === "Proc. 3.2") {
      humanReviewOnly.push(`${procId} | nested cast-out steps are arithmetic-ready, but table/counting behavior remains benchmark-only human review.`);
      continue;
    }

    if (hasOnlyStrong && hasNoHardBlock) {
      readyForArithmetic.push(`${procId} | ready`);
    } else {
      humanReviewOnly.push(`${procId} | blocked`);
    }
  }

  return {
    generated_at: new Date().toISOString(),
    stage: "phase2a_expression_quality_audit",
    total_entries: entries.length,
    backup_summary: backupSummary,
    formalization_summary: formalizationSummary,
    expressions_with_AB_backup: expressionsWithAB,
    expressions_with_CD_backup: expressionsWithCD,
    operation_ids_need_rename: operationIdsNeedRename,
    formal_expressions_with_insufficient_evidence: insufficientEvidence,
    procs_ready_for_arithmetic_validation: readyForArithmetic,
    procs_limited_to_human_review: humanReviewOnly,
    current_pilot_schema_regenerate:
      "No immediate regenerate required. The current Phase 2A pilot schema can stay as the human-readable layer, but a future step-level expression array should be added in a new artifact rather than overwriting the pilot schema.",
  };
}

async function main() {
  const [pilotRaw, reconRaw, baselineRaw] = await Promise.all([
    readJson(INPUT_PILOT),
    readJson(INPUT_RECON),
    readJson(INPUT_BASELINE),
  ]);

  const pilotItems = pilotRaw.items ?? [];
  const reconItems = reconRaw.items ?? [];
  const baselineTargets = baselineRaw.target_proc_status ?? [];

  ensure(
    TARGET_PROC_IDS.every((procId) => pilotItems.some((item) => item.proc_id === procId)),
    "Phase 2A pilot is missing one or more target Proc items."
  );
  ensure(
    TARGET_PROC_IDS.every((procId) => reconItems.some((item) => item.proc_id === procId)),
    "Reconstruction input is missing one or more target Proc items."
  );
  ensure(
    TARGET_PROC_IDS.every((procId) => baselineTargets.some((item) => item.proc_id === procId)),
    "Phase 1 baseline is missing one or more target Proc statuses."
  );

  const pilotByProc = new Map(pilotItems.map((item) => [item.proc_id, item]));
  const reconByProc = new Map(reconItems.map((item) => [item.proc_id, item]));
  const baselineByProc = new Map(baselineTargets.map((item) => [item.proc_id, item]));

  const entries = ENTRY_SPECS.map((spec) => {
    const pilot = pilotByProc.get(spec.proc_id);
    const recon = reconByProc.get(spec.proc_id);
    const baseline = baselineByProc.get(spec.proc_id);

    ensure(pilot, `Missing pilot item for ${spec.proc_id}`);
    ensure(recon, `Missing reconstruction item for ${spec.proc_id}`);
    ensure(baseline, `Missing baseline status for ${spec.proc_id}`);

    return {
      proc_id: spec.proc_id,
      step_id: spec.step_id,
      source_phrase: spec.source_phrase,
      cullen_chinese_phrase: spec.cullen_chinese_phrase,
      cullen_translation_phrase: spec.cullen_translation_phrase,
      cullen_commentary_phrase: spec.cullen_commentary_phrase,
      cullen_worked_example: spec.cullen_worked_example,
      backup_level: spec.backup_level,
      operation_id: spec.operation_id,
      formal_expression: spec.formal_expression,
      variables: spec.variables,
      constants: spec.constants,
      input_bindings: spec.input_bindings,
      output_bindings: spec.output_bindings,
      validation_rule: spec.validation_rule,
      formalization_status: spec.formalization_status,
      uncertainties: spec.uncertainties,
      phase2_role: pilot.phase2_role,
      source_span_id: pilot.source_span_id,
      source_text_snippet: makeSnippet(pilot.source_text, 140),
      reconstruction_quote_integrity_status: recon.quote_integrity_status,
      baseline_phase2_role: baseline.phase2_role,
    };
  });

  const mapPayload = {
    generated_at: new Date().toISOString(),
    stage: "phase2a_cullen_backed_expression_map",
    input_scope: {
      pilot: INPUT_PILOT,
      reconstruction: INPUT_RECON,
      baseline: INPUT_BASELINE,
      target_proc_ids: TARGET_PROC_IDS,
    },
    entries,
  };

  const auditPayload = buildAudit(entries);

  await writeJson(OUTPUT_MAP_JSON, mapPayload);
  await writeJson(OUTPUT_AUDIT_JSON, auditPayload);
  await fs.writeFile(OUTPUT_MAP_MD, renderExpressionMapMarkdown(entries), "utf8");
  await fs.writeFile(OUTPUT_AUDIT_MD, renderAuditMarkdown(auditPayload), "utf8");

  process.stdout.write(
    [
      OUTPUT_MAP_JSON,
      OUTPUT_MAP_MD,
      OUTPUT_AUDIT_JSON,
      OUTPUT_AUDIT_MD,
    ].join("\n")
  );
}

main().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
